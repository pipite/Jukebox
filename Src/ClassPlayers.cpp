...
__fastcall XPlayers::XPlayers(XCore *core, HWND hwnd) {
	...
	if (Core->Settings->Engine == "DirectSound" )    PEngine = new XDirectSound();
	if (Core->Settings->Engine == "MediaFoundation") PEngine = new XMediaFoundation();
	if (Core->Settings->Engine == "FFmpeg" )         PEngine = new XFFmpeg();
