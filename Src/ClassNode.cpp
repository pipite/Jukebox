...
__fastcall XNode::~XNode(void) {
	...
	if ( PParent != NULL ) PParent->Child->Extract(this);
};

void __fastcall XNode::Initialize(void) {
	// Node
	PParent          = NULL;
	Child            = new TList();
	Child->Clear();
...
void __fastcall XNode::Clear() {
	int i;
	if ( !IsSong && Child != NULL ) {
		while ( (i = Child->Count) > 0 ) {
			XNode* childNode = ChildAt(i-1);
			if (childNode != NULL) {
				if (childNode->IsFolder) childNode->Clear();
				delete childNode;
			}
		}
		Child->Clear();
...
void __fastcall XNode::UpdateSongAll(int nbsong) {
	...
	if (PLevel != 0 && PParent != NULL) PParent->UpdateSongAll(nbsong);
}

XNode* __fastcall XNode::ChildAt(unsigned index) {
	if (Child == NULL || Child->Count == 0) return NULL;
...
bool __fastcall XNode::HasAsParent(XNode *node) {
	...
	XNode *dest = this;
	while (dest != NULL && node != NULL && dest->Level >= node->Level) {
...
DWORD* __fastcall XNode::GetWave(void) {
	if (SongNode->PWave == NULL) {
		try {
			SongNode->PWave = new DWORD[1000];
			// Initialize the array to prevent undefined behavior
			memset(SongNode->PWave, 0, 1000 * sizeof(DWORD));
		} catch (...) {
...
TBitmap* __fastcall XNode::GetBitmap(void) {
	if (SongNode->PBitmap == NULL) {
		try {
			SongNode->PBitmap = new TBitmap();
		} catch (...) {
			// Handle allocation failure
			return NULL;
		}
...
XNode* __fastcall XNode::GetRandomChild(void) {
	...
	// Ensure index is within bounds (0 to count-1)
