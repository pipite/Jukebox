...
void __fastcall ThreadFastScan::Execute() {
	...
	for (i = 0; i < PFile->Count ; i++) {
		...
		// Si c'est le nœud racine, utiliser le chemin tel quel
		if (nt == NODETYPE::ROOT) {
			fullPath = s;
		}
		// Si c'est un dossier ou une chanson, ajouter MusicPath au chemin
		else if (nt == NODETYPE::FOLDER || nt == NODETYPE::SONG) {
		...
		if ( (i == 0) && (fullPath != Core->Settings->MusicPath) )  {
			Synchronize( Abort );
			return;
		}
		...
		switch (nt) {
			case NODETYPE::ROOT   : {
				Core->TvList->NRArbo->Path = fullPath;
				Core->TvList->NRFlat->Path = fullPath;
				Core->TvList->NRSong->Path = fullPath;
			...
			case NODETYPE::FOLDER : {
				...
				nf = Core->TvList->NRFlat->Add(fullPath,nt);

				// FlatAz
				a = f[1];
				if (nfaz != NULL) if (nfaz->CountSongAll == 0) delete nfaz; //nfaz->Parent->Extract(nfaz);
				if ( (a >= 'A') && (a <= 'Z') ) {
					nfaz = Core->TvList->NRFlatAz->ChildAt(a - 'A' + 1)->Add(fullPath,nt);
				} else {
					nfaz = Core->TvList->NRFlatAz->ChildAt(0)->Add(fullPath,nt);
				}
				// ArboAz
				if (PLevel == 1) {
					if ( (a >= 'A') && (a <= 'Z') ) {
						naaz[2] = Core->TvList->NRArboAz->ChildAt(a - 'A' + 1)->Add(fullPath,nt);
					} else {
						naaz[2] = Core->TvList->NRArboAz->ChildAt(0)->Add(fullPath,nt);
					}
				} else {
					naaz[PLevel+1] = naaz[PLevel]->Add(fullPath,nt);
				}
				break;
			}
			case NODETYPE::SONG : {
				// Song
				ns = Core->TvList->NRSong->Add(fullPath,nt);
				ns->CountPlay = cp;
				// Arbo
				na[PLevel-1]->Link(ns);
				// Flat
