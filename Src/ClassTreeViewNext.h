...
class XTvNext : public TTreeView {
	...
	void         __fastcall ToggleNode(TTreeNode *node); // Nouvelle méthode pour plier/déplier un nœud
	
	// Nouvelle méthode interne pour la suppression récursive optimisée
	void         __fastcall RemoveInternal(TTreeNode *treenode, bool &needFill);
	...
	void         __fastcall OExpanding(TObject *Sender, TTreeNode *Node, bool &AllowExpansion);

public:
				 XTvNext(TComponent *owner, XCore *core);
				 ~XTvNext(void);
